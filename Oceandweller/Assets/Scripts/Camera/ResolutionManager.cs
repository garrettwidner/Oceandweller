...
public class ResolutionManager : MonoBehaviour
{
    ...
    int[] resolutions = new int[] { 640, 1280, 1920 };
    ...
    public int CurrentResolutionIndex
    {
        get
        {
            return Screen.fullScreen ? currFullscreenRes : currWindowedRes;
        }
    }
    ...
    public int MaxResolutionIndex
    {
        get
        {
            return Screen.fullScreen ? (FullscreenResolutions.Count - 1) : (WindowedResolutions.Count - 1);
        }
    }

    public Vector2 GetResolutionByIndex(int index)
    {
        if (Screen.fullScreen && index <= FullscreenResolutions.Count)
        {
            return FullscreenResolutions[index];
        }
        else if (!Screen.fullScreen && index < WindowedResolutions.Count)
        {
            return WindowedResolutions[index];
        }
        else
        {
            print("Warning: Index must be within the resolutions list count");
            return Vector2.zero;
        }
    }
